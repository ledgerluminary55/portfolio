...
import {ServiceType, CurrentSlideData} from '../../utils/types'

type Props = {
  currentSlideData: CurrentSlideData;
  sliderData: ServiceType[];
  data: ServiceType[];
  transitionData: ServiceType;
  handleData: React.Dispatch<React.SetStateAction<ServiceType[]>>;
  handleTransitionData: React.Dispatch<React.SetStateAction<ServiceType>>;
...
function Controls({
  ...
  const handlePrev = () => {
    ...
    handleCurrentSlideData({
      ...
      index: sliderData.findIndex(
        (ele) => ele.index === data[data.length - 1].index
      ),
    });
  ...
  const handleNext = () => {
    ...
    handleCurrentSlideData({
      ...
      index: sliderData.findIndex((ele) => ele.index === data[0].index),
    });
